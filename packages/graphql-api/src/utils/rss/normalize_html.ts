...
export function normalizeHtml({ html, domain, coverUrl }): string {
  ...
  (function images() {
    $('img').each(function (i) {
      ...
      if (i == 0 && coverUrl && getStringSimilarity(src, coverUrl) > 50) {
        $(this).remove();
      } else {
        ...
        if (!image.src || image.src === coverUrl) {
          ...
          if (image.type === 'set') {
            ...
            if (
              i == 0 &&
              coverUrl &&
              getStringSimilarity(averageSrcSet.url, coverUrl) > 50
            ) {
              $(this).remove();
            } else {
              ...
              $(this).attr(
                'height',
                averageSrcSet.height?.toString() ??
                  ((averageSrcSet.width * 10) / 16).toString()
              );
            }
          } else {
...
function getAverageImageFromSrcSet(
...
function getStringSimilarity(string1: string, string2: string): number {
  ...
  for (let i = 0; i < string1.length; i++) {
    for (let j = 0; j < string2.length; j++) {
      ...
      while (
        string1[i + substringLength] === string2[j + substringLength] &&
        i + substringLength < string1.length &&
        j + substringLength < string2.length
      ) {
        substringLength++;
      }
