...
export function Sidebar() {
  ...
  const [
    ...
    { open: openCreateTopicModal, close: closeCreateTopicModal },
  ] = useDisclosure(false);
  ...
  const topics =
    dataListPublishers?.topicSearch?.edges?.map(
      (edge) => edge && TopicModel.fromGraphQL(edge.node)
    ) ?? [];
  ...
  const handleCreateTopic = async (data: CreateTopicFormData) => {
    ...
    await createTopic({
      ...
      update: (cache, { data }) => {
        const newTopic = data?.topicCreate?.topic;
        if (newTopic) {
          cache.updateQuery<ListUserPublishersQuery>(
            {
              query: ListUserPublishersDocument,
            },
            (data) => {
              if (data) {
                return {
  ...
  return (
    <div className="p-3">
      ...
      <div className="w-full flex justify-center p-2">
        <Button variant="filled" mt={10} onClick={openCreateTopicModal}>
          <PlusIcon className="h-6 w-6" />
        </Button>
      </div>
      {activeTopic && (
      ...
      {userId && (
        <CreateTopicModal
          ...
          isOpen={isOpenedCreateTopicModal}
